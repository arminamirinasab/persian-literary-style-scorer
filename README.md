...
This file contains 5,000 Persian sentences. Each record includes an ID, the sentence text, the score, and the data split:
...
data/persian_literary_5000.json   5,000-sentence training corpus
```
...
These numbers show that the model has reconstructed the patterns in the initial corpus with low error. Since the data currently uses initial labels and may include semi-automated generation or labeling, the high accuracy should be interpreted with caution. The current result is best understood as evidence for the feasibility of the framework, not as a final proof of generalization to all real Persian texts.

## Research Limitations

Several important points should be addressed in future work:

- The labels should be strengthened through multiple human annotators.
...
The current version is usable through a graphical interface and is designed for research presentation, sample analysis, and demonstrating the practical usability of the model.
